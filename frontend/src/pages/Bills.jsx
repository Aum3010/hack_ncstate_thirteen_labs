  ...
  return (
    <div className="bills-page">
      <div className="page-header">
        ...
        <button type="button" className="btn btn-ember" onClick={() => setShowForm(!showForm)}>
          {showForm ? 'Cancel' : 'Add bill'}
        </button>
      </div>
      {error && <div className="auth-error">{error}</div>}
      {showForm && (
        <section className="bills-section" style={{ marginBottom: '1.5rem' }}>
      ...
      <section className="bills-section">
        ...
        <div className="bills-list">
          {bills.length === 0 ? (
            ...
            bills.map((b) => (
              <div key={b.id} className="bill-card-inner">
